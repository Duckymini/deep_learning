...
%%%%%%%%% ADD ABSTRACT %%%%%%%%%
\begin{abstract}
Implicit hate speech — relying on coded language, dog whistles, and irony — remains significantly harder to detect than its explicit counterpart, while content moderation regulations require justifying removal decisions. We propose a Retrieval-Augmented Classifier (RAC) that grounds fine-tuned transformer encoders in two complementary FAISS indexes: labeled precedents from three benchmark datasets and expert knowledge bases. The Example and Full indexes consistently improve over no-index baselines, with best gains of $+$1.3 F1 on IHC (F1\,=\,0.810), $+$2.7 F1 on ISHate (F1\,=\,0.906), and $+$1.5 F1 on Vicomtech (F1\,=\,0.843). On-demand LLaMA~3.3 explanations anchored to retrieved evidence address explainability requirements without involving an LLM in the classification decision.

%%%%%%%%% ADD KEYWORDS %%%%%%%%%
% Make sure to keep keywords on a new line.
\textbf{Keywords:} implicit hate speech, retrieval-augmented classification, explainability, transformer fine-tuning, FAISS
...
Hate speech detection has made substantial progress with fine-tuned transformer encoders, yet implicit hate speech — relying on coded language, dog whistles, and irony — remains significantly harder to detect \cite{elsherief2021latent, ocampo2023indepth}. Unlike explicit forms, detection requires cultural and contextual knowledge beyond lexical patterns.

The EU Digital Services Act now requires platforms to justify content removal decisions, yet existing encoder-based classifiers offer no explanation mechanism. We address both challenges with a RAC: retrieval grounds detection in labeled precedents and expert knowledge, while providing a natural anchor for human-readable explanation. Concretely, we contribute (1) a RAC pipeline combining a Sentence-BERT retriever \cite{reimers2019sbert} with a dual FAISS index (labeled examples + expert glossaries) feeding a fine-tuned encoder; (2) a systematic evaluation across 3 encoders, 3 datasets, and 3 index configurations; (3) a hyperparameter study ($k$, $\tau$) demonstrating low retrieval sensitivity; and (4) on-demand LLaMA~3.3 \cite{dubey2024llama3} explanations grounded in retrieved evidence, decoupled from the classification decision.
...
ElSherief et al.~\cite{elsherief2021latent} introduce IHC and find that integrating external knowledge graphs yields no performance gain, while models suffer from identity-term bias.
...
Policy-grounded RAG systems applied to hate speech exist but rely on a generative LLM as the final decision-maker, increasing inference cost and risking hallucinated or weakly grounded decisions when policy documents are incomplete.
...
Our RAC reaches competitive performance (F1\,=\,81.0) with a 125M-parameter encoder, adding retrieval-grounded explainability at a 70$\times$ memory reduction.
...
All text is encoded with \textbf{Sentence-BERT}~\cite{reimers2019sbert} and stored in L2-normalised \textbf{FAISS} indexes. We construct three indexes: \textbf{Example (E)}, label-prefixed chunks from training splits of all three datasets; \textbf{Knowledge (K)}, chunks from the expert glossaries; and \textbf{Full (E+K)}, their union. To prevent data leakage, each training example is masked from retrieval by its chunk ID.
...
We fine-tune three encoders: \textbf{BERT} (110M parameters), \textbf{RoBERTa} (125M parameters), and \textbf{HateBERT}~\cite{caselli2021hatebert}, pre-trained on abusive language corpora. Each is fine-tuned with a binary classification head for 3 epochs (lr\,=\,$2{\times}10^{-5}$, batch size 16, seed 42). To ensure full reproducibility, the random seed is explicitly set before each \texttt{from\_pretrained} call, fixing the classifier head initialisation independently of the training loop seed.

\subsection{Hyperparameter Tuning}
We grid-search $k \in \{3, 5, 10\}$ and $\tau \in \{0.3, 0.4, 0.5, 0.6\}$ on IHC with the Full index (Tab.~\ref{tab:hp_tuning}, Appendix). Optimal values are applied to all subsequent experiments.

\subsection{On-demand Explanation}
At inference time, the augmented input and the encoder's prediction are passed to \textbf{LLaMA~3.3}~\cite{dubey2024llama3} to generate a grounded, human-readable justification. The LLM is not involved in the classification decision and is not fine-tuned; it synthesises retrieved evidence into a natural-language explanation on demand.


\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{model_overview.pdf}
...
\textbf{IHC and ISHate.} On IHC, RoBERTa(E) achieves the best result of F1\,=\,0.810 ($+$1.3 pp over its no-index baseline). On ISHate, RoBERTa(E+K) achieves F1\,=\,0.906 ($+$2.7 pp), with the Full index outperforming the Example-only index. Retrieved labelled examples act as effective few-shot anchors grounding the encoder in semantically similar precedents.

\textbf{Vicomtech.} On Vicomtech (1,914 training examples), RoBERTa(E) achieves F1\,=\,0.843 ($+$1.5 pp over baseline). HateBERT(E+K)\,=\,0.840 is competitive, suggesting combining both indexes provides marginal additional signal when in-distribution coverage is sparse.
...
BERT (E)            & 0.789 & 0.886 & 0.825 \\
BERT (K)            & 0.763 & 0.868 & 0.784 \\
BERT (E+K)          & 0.783 & 0.887 & 0.825 \\
\midrule
...
RoBERTa (E)         & \textbf{0.810} & 0.903 & \textbf{0.843} \\
RoBERTa (K)         & 0.784 & 0.887 & 0.754 \\
RoBERTa (E+K)       & 0.800 & \textbf{0.906} & 0.839 \\
\bottomrule
\end{tabular}}
...
We train and evaluate on IHC while excluding all IHC training examples from the index. Macro F1 does not degrade meaningfully, confirming that the retriever generalises via semantic similarity rather than exploiting in-distribution shortcuts.
...
\begin{minipage}{\dimexpr\columnwidth-2\fboxsep-2\fboxrule\relax}\small
  ...
  {\footnotesize Label: hate \;|\; Prediction: hate \;|\; Confidence: 86.6\%}
  ...
  \begin{tabular}{@{} r l p{0.78\linewidth} @{}}
    ...
    1 & 0.445 & [Hate] Zionist: Hatebase entry for Zionist. Target category: Judaism. Filter group: religion. \\[1pt]
  ...
  The content was flagged as hate speech due to the mention of \textit{zog},
  associated with offensive language targeting the Jewish community.
  The context suggests a call to action perceived as threatening or discriminatory.
  \textbf{Target group:} Judaism.
\end{minipage}%
}
...
Our RoBERTa(E+K) requires 0.88\,GB of GPU memory vs.\ 62.4\,GB for NV-Embed~\cite{cheremetiev2025specializing}, which achieves marginally higher F1 (81.68 vs.\ 81.0). This 70$\times$ memory reduction makes the framework substantially more practical for large-scale deployment under DSA cost constraints.



\subsection{Limitations}
\label{sec:limits}
\textbf{Annotation subjectivity.} Implicit hate speech lacks a universal definition, leading to annotator disagreement and identity-term bias --- models over-classify tweets mentioning group identifiers as hateful.
...
\centering
\captionof{table}{RAC fine-tuning results. E\,=\,Example index, K\,=\,Knowledge index, E+K\,=\,Full index. Bold indicates best per column.}
...
BERT (E)            & 0.789 & 0.798 & 0.783 & 0.886 & 0.883 & 0.889 & 0.825 & 0.834 & 0.826 \\
BERT (K)            & 0.763 & 0.763 & 0.763 & 0.868 & 0.865 & 0.871 & 0.784 & 0.785 & 0.785 \\
BERT (E+K)          & 0.783 & 0.786 & 0.781 & 0.887 & 0.885 & 0.891 & 0.825 & 0.834 & 0.826 \\
\midrule
...
HateBERT (E+K)      & 0.778 & 0.783 & 0.774 & 0.898 & 0.896 & 0.900 & 0.840 & \textbf{0.851} & 0.841 \\
\midrule
RoBERTa (No-index)  & 0.797 & 0.802 & 0.793 & 0.879 & 0.877 & 0.882 & 0.828 & 0.829 & 0.828 \\
RoBERTa (E)         & \textbf{0.810} & \textbf{0.824} & \textbf{0.802} & 0.903 & 0.901 & 0.907 & \textbf{0.843} & 0.844 & \textbf{0.843} \\
RoBERTa (K)         & 0.784 & 0.798 & 0.776 & 0.887 & 0.885 & 0.889 & 0.754 & 0.796 & 0.762 \\
RoBERTa (E+K)       & 0.800 & 0.809 & 0.795 & \textbf{0.906} & \textbf{0.903} & \textbf{0.909} & 0.839 & 0.839 & 0.839 \\
\bottomrule
\end{tabular}}
...
\toprule
Model & $k$ & $\tau{=}0.3$ & $\tau{=}0.4$ & $\tau{=}0.5$ & $\tau{=}0.6$ \\
\midrule
...
\toprule
Dataset & S\,0 & S\,1 & S\,2 & S\,42 & Mean & Std \\
\midrule
...
\centering
\captionof{table}{IHC binary comparison with ElSherief et al.~\cite{elsherief2021latent}. Evaluated on not\_hate vs.\ implicit\_hate only, matching their Table~3 setup.}
